...
4. Import or select the signed runtime release and select **Scan controller for review**. Scanning is disabled while import is in progress. The session pins the selected digest; another administrator importing a release cannot change this job or its retries.
...
| SSH authentication or `sudo` fails          | If the factory login changed, select **Advanced: use different SSH credentials**. Check privileges and reachability before retrying.                                                                                                   |
...
- In-process operations are sequenced per controller, and a controller-side `flock` spans transfer/staging/replacement. Installation journals retain interrupted work for explicit cleanup.
- Compose ATT-983 / PR #1802's shared `context.audit` bridge; this code supplies no second audit sink. Automatic claim carries the persisted authenticated initiator. For HTTP unclaim, keep its single audit wrapper inside `removeControllerSafely(id, assertOwned => audit.run(..., () => wago.remove(id, assertOwned)))`. Audit receipt availability is independent of operation success; no durable storage is invented when the host reports unavailable.
...
Do not use these gaps as reasons to bypass host-key, signed-bundle, or MQTT identity/credential checks. Escalate them through the release process and attach hardware evidence to [ATT-984](https://linear.app/attraccess/issue/ATT-984/validate-the-four-wago-package-assemblies).
