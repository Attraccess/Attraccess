...
export const useUsersServiceCorrectSetupEmail = <TData = Common.UsersServiceCorrectSetupEmailMutationResult, TError = unknown, TContext = unknown>(options?: Omit<UseMutationOptions<TData, TError, {
  requestBody: CorrectSetupEmailDto;
}, TContext>, "mutationFn">) => useMutation<TData, TError, {
  requestBody: CorrectSetupEmailDto;
}, TContext>({ mutationFn: ({ requestBody }) => UsersService.correctSetupEmail({ requestBody }) as unknown as Promise<TData>, ...options });
