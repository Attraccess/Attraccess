...
- **Option A implemented**: `POST resources/:resourceId/maintenances/:maintenanceId/finish` with optional body `FinishMaintenanceDto { notes?: string }`. Controller uses `@CanManageMaintenance()`; validates maintenance belongs to resource and is not already finished.
- **Service**: `finishMaintenance(maintenanceId, options?: { userId?: number; notes?: string })` sets `endTime`, `completedAt`, and `completedByUser` from `options.userId`; emits `ResourceMaintenanceChangedEvent`. Notes accepted in DTO; persistence of notes (e.g. new column) left to work item 06 if desired.
- **Frontend**: Mark as done button (CheckCircle icon) for active maintenances in `apps/frontend/src/app/resources/details/maintenance-management/index.tsx`; calls `useResourceMaintenancesServiceFinishMaintenance`, invalidates maintenances list on success. Translations added for `actions.markDone.title` (EN/DE).
- **API client**: `finishMaintenance` and `useResourceMaintenancesServiceFinishMaintenance` added to `libs/react-query-client` (types + service + query). Regenerate with `nx run api-client:generate` and `nx run react-query-client:generate` when workspace build succeeds to refresh from OpenAPI.
