...
try {
  ...
  try {
    // docker run can create the container before failing or timing out, without
    // returning its ID. Recover by name only after verifying this run's label.
    if (!container) container = await recoverOwnedContainer();
    if (container) {
