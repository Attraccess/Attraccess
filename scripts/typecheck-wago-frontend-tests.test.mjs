...
async function runCompilerFixture({ stdout = formerDiagnostics, stderr = '', status = 1 } = {}) {
...
test('rejects both former commissioning exceptions without invoking the package manager', async () => {
  const result = await runCompilerFixture();
  assert.equal(result.status, 1);
  assert.equal(result.stderr, formerDiagnostics);
  assert.equal(result.stdout, '');
});

test('rejects a new TypeScript diagnostic alongside the formerDiagnostics', async () => {
  const stdout = formerDiagnostics + 'frontend/new.ts(1,1): error TS2322: Type mismatch.\n';
...
test('rejects a changed commissioning diagnostic', async () => {
  const stdout = formerDiagnostics.replace('(108,88)', '(109,88)');
  const result = await runCompilerFixture({ stdout });
  assert.equal(result.status, 1);
  assert.equal(result.stderr, stdout);
});
...
test('does not whitelist package-manager warnings if they appear in compiler output', async () => {
  ...
  assert.equal(result.status, 1);
  assert.equal(result.stderr, formerDiagnostics + warning);
});
...
test('accepts a successful compiler with no diagnostics', async () => {
  ...
  assert.match(result.stdout, /Raw frontend test tsc exit: 0; no diagnostics/);
});
