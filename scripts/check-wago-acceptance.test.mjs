...
const fixture = () => ({
  ...
  controller: {
    ...
    hardwareId: 'fixture',
    startingState: 'fresh',
    wiringEvidence: 'fixture-ref',
  },
  build: {
    pluginCommit: 'a'.repeat(40),
    runtimeCommit: 'b'.repeat(40),
    frontendDigest: `sha256:${'c'.repeat(64)}`,
    backendDigest: `sha256:${'d'.repeat(64)}`,
    runtimeDigest: `sha256:${'e'.repeat(64)}`,
    protocolVersion: '1.0.0',
    signedBundleEvidence: 'fixture-ref',
    visualArtifactProvisioningEvidence: 'fixture-ref',
  },
...
test('rejects simulation, implementer participation, intervention and blockers', () => {
  for (const patch of [
    ...
    { participant: { id: ' IMPLEMENTER ', experience: 'nontechnical' } },
    { noCodeJourney: false },
    { developerIntervention: true },
    { blockers: ['credentials unbacked'] },
    { safeFixtureConfirmed: false },
    { obstacles: undefined },
  ])
...
test('every required check must pass with a recorded observation and evidence', () => {
  for (const id of requiredChecks) {
    for (const patch of [
      ...
      Object.assign(
        evidence.checks.find((check) => check.id === id),
        patch,
      );
