...
class ConfigurationBrowser(unittest.TestCase):
    ...
    def test_modbus_save_publish_rollback_retains_binding_and_profile(self):
        ...
        profile = self.dialog.locator("details").filter(has=self.page.locator("summary", has_text="WAGO 879-3000"))
        profile.locator("summary").click()
        profile.get_by_role("button", name=re.compile("^Duplicate WAGO 879-3000")).click()
        self.choose("Device profile", "WAGO 879-3000 — UNQUALIFIED / map unverified (custom) v1")
        self.save()
        ...
        point = original["physicalPoints"][0]
        channel = original["logicalChannels"][0]
        self.assertEqual(point["hardwareProfile"], "modbus")
        self.assertEqual(point["modbus"]["measurementId"], "active-power")
        self.assertEqual(channel["measurement"], {"unit": "watt", "kind": "live", "scale": 1, "offset": 0})
        self.assertEqual(self.fixture.count("/publish"), 0)
        self.publish()
        expect(self.dialog.get_by_text("Publication submitted for revision 1. Current controller report is shown in revision history.", exact=True)).to_be_visible()
        self.assertEqual(json.loads(self.fixture.revisions[0]["snapshot"]), original)
        self.choose("Named measurement", "Imported energy (watt-hour)")
        self.save()
        changed = json.loads(self.fixture.draft["snapshot"])
        self.assertEqual(changed["physicalPoints"][0]["id"], point["id"])
        self.assertEqual(changed["logicalChannels"][0]["id"], channel["id"])
        ...
        self.assertTrue(rollback["sourceHash"])
        self.assertTrue(rollback["currentHash"])
        self.assertTrue(rollback["draftHash"])
        expect(self.dialog.get_by_role("button", name=re.compile("Named measurement"))).to_contain_text("Active power")
        self.dialog.get_by_role("button", name=re.compile("Named measurement")).scroll_into_view_if_needed()
        ...
        self.assertLessEqual(self.dialog.evaluate("el => el.scrollWidth - el.clientWidth"), 1)

    def test_modbus_builtin_duplicate_and_invalid_custom_map(self):
        self.add_modbus_meter()
        profile = self.dialog.locator("details").filter(has=self.page.locator("summary", has_text="WAGO 879-3000"))
        profile.locator("summary").click()
        expect(profile.get_by_role("textbox", name="Profile name", exact=True)).to_be_disabled()
        expect(profile.get_by_role("textbox", name="Register address (decimal)").first).to_be_disabled()
        profile.get_by_role("button", name=re.compile("^Duplicate WAGO 879-3000")).click()
        custom = self.dialog.locator("details").filter(has=self.page.locator("summary", has_text="(custom)"))
        ...
        self.choose("Device profile", "Workshop custom map v1")
        self.save()
        ...
        self.assertEqual(saved["physicalPoints"][0]["modbus"]["measurementId"], "active-power")
        custom.get_by_role("textbox", name="Profile name", exact=True).scroll_into_view_if_needed()
        self.assertLessEqual(custom.evaluate("el => el.scrollWidth - el.clientWidth"), 1)
    ...
    def test_modbus_review_preserves_input_policy_and_repairs_output_conversion(self):
        ...
        self.dialog.get_by_role("spinbutton", name=re.compile(r"^Watchdog timeout \(ms\)"), exact=False).fill("2345")
        self.dialog.get_by_text("Expected value range", exact=True).click()
        self.dialog.get_by_role("spinbutton", name=re.compile(r"^Maximum"), exact=False).fill("1000")
        self.choose("Named measurement", "Imported energy (watt-hour)")
        expect(self.dialog.get_by_role("spinbutton", name=re.compile(r"^Watchdog timeout \(ms\)"), exact=False)).to_have_value("2345")
        self.save()
        first = json.loads(self.fixture.draft["snapshot"])
        self.assertEqual(first["logicalChannels"][0]["disconnectPolicy"], {"mode": "watchdog", "timeoutMs": 2345})
        self.assertEqual(first["logicalChannels"][0]["range"], {"minimum": 0, "maximum": 1000})
        profile = self.dialog.locator("details").filter(has=self.page.locator("summary", has_text="WAGO 879-3000"))
        profile.locator("summary").click()
        profile.get_by_role("button", name=re.compile("^Duplicate WAGO 879-3000")).click()
        custom = self.dialog.locator("details").filter(has=self.page.locator("summary", has_text="(custom)"))
        custom.locator("summary").click()
        custom.get_by_role("button", name="Add action", exact=True).click()
        self.choose("Device profile", "WAGO 879-3000 — UNQUALIFIED / map unverified (custom) v1")
        self.choose("Named action", "Switch")
        expect(self.dialog.get_by_role("spinbutton", name=re.compile(r"^Maximum"), exact=False)).to_have_value("1000")
        self.choose("Named measurement", "None")
        expect(self.dialog.get_by_role("spinbutton", name=re.compile(r"^Maximum"), exact=False)).to_have_count(0)
        self.save()
        converted = json.loads(self.fixture.draft["snapshot"])
        self.assertEqual(converted["logicalChannels"][0]["id"], first["logicalChannels"][0]["id"])
        ...
        self.assertEqual(self.fixture.count("/publish"), 0)
        self.dialog.get_by_role("button", name=re.compile("Named action")).scroll_into_view_if_needed()
    ...
    def test_modbus_review_orphan_can_rebind_or_release_after_device_deletion(self):
        ...
        # A valid saved snapshot may contain unassigned points; seed only the in-memory fixture.
        self.fixture.draft["snapshot"] = json.dumps(orphan)
        self.button("Close").click()
        self.open_editor()
        self.button("Remove device").click()
        expect(self.button("Save draft")).to_be_disabled()
        unused = self.dialog.get_by_role("region", name="Unused physical assignments", exact=True)
        expect(unused).to_be_visible()
        self.button("Add device").click()
        ...
        self.save()
        repaired = json.loads(self.fixture.draft["snapshot"])
        self.assertEqual(repaired["physicalPoints"][0]["id"], orphan["physicalPoints"][0]["id"])
        self.assertEqual(repaired["physicalPoints"][0]["modbus"]["deviceId"], repaired["modbus"]["devices"][0]["id"])
        self.assertEqual(repaired["logicalChannels"], [])
        unused.scroll_into_view_if_needed()
        self.assertLessEqual(unused.evaluate("el => el.scrollWidth - el.clientWidth"), 1)
        bounds = unused.bounding_box()
        self.assertGreaterEqual(bounds["x"], -1)
        self.assertLessEqual(bounds["x"] + bounds["width"], self.viewport["width"] + 1)
        self.page.screenshot(path=str(self.artifacts / "repaired-orphan.png"), full_page=True, animations="disabled")
        self.button("Remove device").click()
        expect(self.button("Save draft")).to_be_disabled()
        unused.get_by_role("button", name=re.compile("^Release ")).click()
        self.save()
