...
it('keeps verification/recovery reachable while allowing a claimed controller to be configured', () => {
  ...
  const controller = { id: 1, hardwareId: 'fixture', name: 'Fixture', trustState: 'claimed', connectivity: 'online' } as WagoController;
  const onResume = vi.fn();
  const onConfigure = vi.fn();
  render(<ControllersTable controllers={[controller]} sessions={[session]} onResume={onResume}
    onConfigure={onConfigure} onClaim={vi.fn()} onRemove={vi.fn()} />);
