...
function DevicePassport({ className, name, step }: { className?: string; name: string; step: number }) {
  return <aside className={`wg:min-w-0 wg:rounded-large wg:bg-default-100 wg:p-5 ${className ?? ''}`}><CpuIcon className="wg:h-10 wg:w-10 wg:text-accent-soft-foreground" /><p className="wg:mt-4 wg:text-xs wg:font-semibold wg:uppercase wg:tracking-wider wg:text-muted">CC100 device passport</p><p className="wg:mt-1 wg:truncate wg:text-lg wg:font-semibold">{name}</p><div className="wg:mt-5 wg:space-y-3"><PassportRow label="Identity" value={step >= 2 ? 'Verified automatically' : 'Not scanned'} /><PassportRow label="Runtime" value={step >= 3 ? 'Provisioning' : 'Not delivered'} /><PassportRow label="Claim" value={step >= 3 ? 'Automatic' : 'Queued'} /></div><div className="wg:mt-6 wg:flex wg:gap-1">{[0, 1, 2, 3].map((index) => <span key={index} className={`wg:h-1.5 wg:flex-1 wg:rounded-full ${index <= step ? 'wg:bg-accent' : 'wg:bg-default-300'}`} />)}</div></aside>;
}
