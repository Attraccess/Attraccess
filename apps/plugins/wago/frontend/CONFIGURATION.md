...
Publication review identities bind the saved snapshot and editor metadata, current
revision, and sorted affected flow references. Forced publication rejects a changed
impact set until it is reviewed again. The opaque rollback `draftHash` additionally
binds source/current revision numbers and metadata; it is not a runtime content hash.
Modbus endpoint, unit, mapped register and transform changes contribute to bound
channel impacts and guard/feedback consumers.
