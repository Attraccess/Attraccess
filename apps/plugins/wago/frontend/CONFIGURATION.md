...
ATT-983 audit integration remains necessary: capture preset before/after counts and provenance inside the configuration lock, and connect forced-publication and rejection-acknowledgement hooks. Editor metadata alone is not durable audit evidence. ATT-979/1059 retains ownership of Modbus model, transport, and profile fields. Integration of its committed `6f3a405d` stack requires runtime-owner composition: preserve both extracted output/revision barriers and Modbus routing uncertainty protections before mounting its editor in the same local snapshot/save lifecycle.
