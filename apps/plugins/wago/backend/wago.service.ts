...
export class WagoService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    const enrollments = await this.enrollments.createQueryBuilder('enrollment').where('enrollment.consumedAt IS NULL').getMany();
  ...
  private async claimController(
    ...
    const previousController = { trustState: controller.trustState, name: controller.name, mqttServerId: controller.mqttServerId, updatedAt: controller.updatedAt };
    try {
      ...
      controller.trustState = 'claimed'; controller.name = name.trim(); controller.mqttServerId = selectedServerId; controller.updatedAt = new Date().toISOString();
      ...
      if (!credentialDelivered) {
        await this.context.getMqttCredentialProvisioning().revoke({ mqttServerId: selectedServerId, identity, username: identity, vhost: '/' }).catch(() => undefined);
