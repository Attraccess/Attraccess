...
export class AddWagoCommissioningPrincipal1780000000009 implements MigrationInterface {
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    const rows = await queryRunner.query(
      'SELECT COUNT(*) AS count FROM "plugin_wago_commissioning_sessions" WHERE "docker_provision_token" IS NOT NULL',
    );
    if (Number(rows[0].count) !== 0)
      throw new Error('Recover Docker provisioning before removing its saved ownership tokens');
