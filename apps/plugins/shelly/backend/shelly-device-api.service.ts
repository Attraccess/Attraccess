...
export class ShellyDeviceApiService {
  async getDeviceInfo(target: DeviceTarget & DeviceCredentials): Promise<ShellyDeviceInfo> {
    const [status, config] = await Promise.all(
      target.generation === 1
        ? [
            this.getJson(`http://${target.ipAddress}/status`, target),
            this.getJson(`http://${target.ipAddress}/settings`, target),
          ]
        : [
            this.getJson(`http://${target.ipAddress}/rpc/Shelly.GetStatus`, target),
            this.getJson(`http://${target.ipAddress}/rpc/Shelly.GetConfig`, target),
          ]
    );
  ...
  async setAdminPassword(input: SetAdminPasswordInput): Promise<void> {
    ...
    if (input.generation === 1) {
      ...
      await this.getJson(url.toString(), input);
      return;
    }

    const info = (await this.getJson(`http://${input.ipAddress}/shelly`, input)) as { id?: string };
    const realm = info.id?.trim();
    if (!realm) {
      throw new Error('Shelly.SetAuth requires the device id from GET /shelly');
    }

    await this.postJson(`http://${input.ipAddress}/rpc/Shelly.SetAuth`, {
      user: username,
      realm,
  ...
  private postJson(url: string, body: unknown, credentials: DeviceCredentials): Promise<unknown> {
  ...
  private buildAuthHeader(response: Response, url: string, method: string, credentials: DeviceCredentials): Record<string, string> {
...
function buildDigestAuthorization(challenge: string, url: string, method: string, username: string, password: string): string {
