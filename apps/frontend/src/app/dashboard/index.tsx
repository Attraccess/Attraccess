...
function SortablePageCard({ id, className, children }: { id: string; className?: string; children: React.ReactNode }) {
  ...
  return <Card ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={className}>{React.Children.map(children, (child) => React.isValidElement(child) && child.type === GripVerticalIcon ? React.cloneElement(child as React.ReactElement<React.SVGProps<SVGSVGElement>>, { ...attributes, ...listeners }) : child)}</Card>;
}
