...
import { DoorOpen, Plus, Shapes } from 'lucide-react';
...
export function CreateResourceDrawer({ testId }: { testId?: string }) {
  ...
  return (
    <>
      ...
      <StandardDrawer
        isOpen={isOpen}
        onOpenChange={(open) => {
          if (!createResource.isPending) setOpen(open);
        }}
      >
        <DrawerHeader>
        ...
        <DrawerFooter className="flex flex-col gap-2 min-[380px]:flex-row">
          <Button variant="outline" onPress={() => setOpen(false)} isDisabled={createResource.isPending}>
            {t('cancel')}
          </Button>
          <SplitActionButton
