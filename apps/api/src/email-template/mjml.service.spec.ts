...
describe('MjmlService', () => {
  ...
  describe('convertToHtml', () => {
    it('should convert valid MJML to HTML', () => {
      ...
      const result = service.convertToHtml(mjml);
    ...
    it('should return errors for invalid MJML tags', () => {
    ...
    it('should throw BadRequestException for completely broken input', () => {
      expect(() => service.convertToHtml(null as unknown as string)).toThrow(BadRequestException);
    });
  });

  describe('validateAndConvert', () => {
    it('should return HTML for valid MJML', () => {
      const mjml = '<mjml><mj-body><mj-section><mj-column><mj-text>Test</mj-text></mj-column></mj-section></mj-body></mjml>';
      const html = service.validateAndConvert(mjml);

      expect(html).toContain('Test');
      expect(typeof html).toBe('string');
    });

    it('should throw BadRequestException for MJML with validation errors', () => {
      const mjml = '<mjml><mj-body><mj-invalid-tag>Hello</mj-invalid-tag></mj-body></mjml>';

      expect(() => service.validateAndConvert(mjml)).toThrow(BadRequestException);
    });
  });
});
