...
describe('PluginClassificationService', () => {
  ...
  it('recognizes Attraccess plugin packages published on npm', () => {
    expect(service.classify('@attraccess/plugin-example', 'https://registry.npmjs.org', 'attraccess')).toEqual({
      kind: 'official',
  ...
  it('does not trust an official package name from a different registry', () => {
    expect(service.classify('@attraccess/plugin-example', 'https://registry.example.com', 'attraccess').kind).toBe(
      'community',
    );
  });

  it('rejects a plugin package whose publisher does not match', () => {
    expect(service.classify('@attraccess/plugin-example', 'https://registry.npmjs.org', 'someone-else').kind).toBe(
      'community',
    );
  });

  it('rejects a plugin package when the registry does not expose a publisher', () => {
    expect(service.classify('@attraccess/plugin-other', 'https://registry.npmjs.org').kind).toBe('community');
  });
