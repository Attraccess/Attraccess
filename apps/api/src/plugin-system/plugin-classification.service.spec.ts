...
describe('PluginClassificationService', () => {
  ...
  it('rejects an official package source whose registry publisher does not match', () => {
    expect(service.classify('@attraccess-plugins/shelly', 'https://registry.npmjs.org', 'someone-else').kind).toBe(
      'community',
    );
  });

  it('allows an approved source when the registry does not expose a publisher', () => {
    expect(service.classify('@attraccess-plugins/rabbitmq', 'https://registry.npmjs.org').kind).toBe('official');
  });
