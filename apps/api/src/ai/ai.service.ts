    ...
    const result = streamText({
      ...
      onFinish: async ({ text, steps, finishReason, usage }) => {
        ...
        for (const tc of toolCalls) {
          this.logger.debug(`  Tool call: ${tc.toolName}(${JSON.stringify((tc as Record<string, unknown>).args).slice(0, 200)})`);
        }
        for (const tr of toolResults) {
          const resultStr = JSON.stringify((tr as Record<string, unknown>).result ?? tr).slice(0, 300);
