...
describe('ResourceGroupsIntroductionsController', () => {
  ...
  beforeEach(() => {
    ...
    controller = new ResourceGroupsIntroductionsController(
      resourceGroupsIntroductionsService as ResourceGroupsIntroductionsService,
    );
  });
  ...
  it.each(['grant', 'revoke'] as const)(
    ...
    async (action) => {
      ...
      expect(resourceGroupsIntroductionsService[action]).toHaveBeenCalledWith(5, 3, data, { performedByUserId: 9, ...(action === 'grant' ? { auditOrigin: { actorId: 9, authenticationMethod: undefined } } : {}) });
    },
  );
});
