...
export class CompanionUnlockPcExecutor implements NodeExecutor {
  ...
  async execute(node: ResourceFlowNode, input: object): Promise<NodeProcessingResult> {
    ...
    if (!parsed.success) {
      throw new FlowExecutionError(`Invalid companion-unlock-pc node data: ${parsed.error.message}`);
    }

    const delivered = await this.companionGateway.sendUnlockCommand(parsed.data.deviceId);
