...
describe('ResourceFlowsExecutorService – handleResourceSessionStartedEvent', () => {
  ...
  it('should skip flow execution for Usage actions', async () => {
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.Usage));
    await service.handleResourceSessionStartedEvent(event);
    expect(triggerResourceUsageNodeSpy).not.toHaveBeenCalled();
  });

  it('should trigger flow for DoorLock action', async () => {
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.DoorLock));
    await service.handleResourceSessionStartedEvent(event);
  ...
  it('should trigger flow for DoorUnlock action', async () => {
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.DoorUnlock));
    await service.handleResourceSessionStartedEvent(event);
    expect(triggerResourceUsageNodeSpy).toHaveBeenCalledWith(
      1,
  ...
  it('should trigger flow for DoorUnlatch action', async () => {
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.DoorUnlatch));
    await service.handleResourceSessionStartedEvent(event);
    expect(triggerResourceUsageNodeSpy).toHaveBeenCalledWith(
      1,
  ...
  it('should re-throw FlowExecutionError from DoorLock flow with correct type', async () => {
    ...
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.DoorLock));
    await expect(service.handleResourceSessionStartedEvent(event)).rejects.toThrow(FlowExecutionError);
  });

  it('should re-throw FlowExecutionError from DoorLock flow with correct message', async () => {
    triggerResourceUsageNodeSpy.mockRejectedValueOnce(new FlowExecutionError('Door access denied'));
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.DoorLock));
    await expect(service.handleResourceSessionStartedEvent(event)).rejects.toThrow('FLOW_EXECUTION_ERROR: Door access denied');
  });
  ...
  it('should re-throw generic errors from door flow', async () => {
    ...
    const event = new ResourceSessionStartedEvent(makeUsage(ResourceUsageAction.DoorLock));
    await expect(service.handleResourceSessionStartedEvent(event)).rejects.toThrow('Database unavailable');
  });
  ...
  it('should log error before re-throwing', async () => {
    ...
    await expect(service.handleResourceSessionStartedEvent(event)).rejects.toThrow();
  ...
  it('should use endTime as event timestamp when available', async () => {
    ...
    const event = new ResourceSessionStartedEvent(usage);
