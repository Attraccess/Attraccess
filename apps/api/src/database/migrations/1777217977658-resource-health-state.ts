...
const SNAPSHOT_TYPES =
  "('input.button','input.resource.usage.started','input.resource.usage.stopped','input.resource.usage.takeover','input.resource.door.unlocked','input.resource.door.locked','input.resource.door.unlatched','input.mqtt.message.received','input.resource.activity.no-activity','output.http.sendRequest','output.mqtt.sendMessage','output.resource.billing.calculation.set-additional-items','output.resource.usage.end-session','output.resource.activity.track-activity','processing.wait','processing.if','processing.set-payload','processing.mqtt.waitForMessage','processing.error')";
...
export class ResourceHealthState1777217977658 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "resource_health_state" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "resourceId" integer NOT NULL, "identifier" text NOT NULL DEFAULT (''), "status" varchar CHECK( "status" IN ('healthy','unhealthy') ) NOT NULL DEFAULT ('healthy'), "reason" text, "source" varchar CHECK( "source" IN ('payload','heartbeat','manual') ) NOT NULL DEFAULT ('manual'), "lastReportedAt" datetime NOT NULL, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), CONSTRAINT "UQ_resource_health_resource_identifier" UNIQUE ("resourceId", "identifier"), CONSTRAINT "FK_resource_health_state_resource" FOREIGN KEY ("resourceId") REFERENCES "resource" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "resource_flow_node" ("id" text PRIMARY KEY NOT NULL, "type" varchar CHECK( "type" IN ${SNAPSHOT_TYPES} ) NOT NULL, "data" json, "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "resourceId" integer NOT NULL, "positionX" integer NOT NULL, "positionY" integer NOT NULL, CONSTRAINT "FK_ca3080b2dbc9c7c88a4a64c469d" FOREIGN KEY ("resourceId") REFERENCES "resource" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
      `INSERT INTO "resource_flow_node"("id", "type", "data", "createdAt", "updatedAt", "resourceId", "positionX", "positionY") SELECT "id", "type", "data", "createdAt", "updatedAt", "resourceId", "positionX", "positionY" FROM "temporary_resource_flow_node" WHERE "type" NOT IN ('output.resource.health.heartbeat','output.resource.health.set')`,
    );
