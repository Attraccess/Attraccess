...
export class RefreshDefaultEmailLayout1783700000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const originalValue = layout.value;
    const hash = createHash('sha256').update(originalValue.replace(/\r\n?/g, '\n')).digest('hex');
    if (hash !== ORIGINAL_LAYOUT_SHA256) {
      return;
    }

    // The original value and timestamps make an administrator edit after the read win.
    await queryRunner.query(
      `UPDATE "setting" SET "value" = ? WHERE "parent" = ? AND "key" = ? AND "createdAt" = ? AND "updatedAt" = ? AND "value" = ?`,
      [readDefaultLayoutBody(), 'email_layout', 'body', layout.createdAt, layout.updatedAt, originalValue],
    );
  }
