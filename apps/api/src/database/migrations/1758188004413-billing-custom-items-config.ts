...
export class BillingCustomItemsConfig1758188004413 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "billing_transaction_item" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "billingTransactionId" integer NOT NULL, "name" varchar NOT NULL, "description" varchar, "externalReference" varchar, CONSTRAINT "FK_2455de178cbdab7028200076b03" FOREIGN KEY ("billingTransactionId") REFERENCES "billing_transaction" ("id") ON DELETE CASCADE ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
      `INSERT INTO "billing_transaction_item"("id", "billingTransactionId", "name", "description", "externalReference") SELECT "id", "billingTransactionId", "name", "description", "externalReference" FROM "temporary_billing_transaction_item"`,
    );
