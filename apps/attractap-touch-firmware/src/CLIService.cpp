...
ParsedCommand CommandParser::parse(const String &input)
{
    ...
    if (secondSpace == -1)
    {
        ...
        cmd.payload = "";
        cmd.isValid = (cmd.type != CMD_INVALID);
        if (!cmd.isValid)
        {
            cmd.errorMessage = "invalid_command_type";
        }
        return cmd;
    }
    ...
    String cmdPart = trimmedInput.substring(0, firstSpace);
    String typePart = trimmedInput.substring(firstSpace + 1, secondSpace);
    String remaining = trimmedInput.substring(secondSpace + 1);
    ...
    // Validate type part is not empty
    if (typePart.length() == 0)
    {
        cmd.isValid = false;
        cmd.errorMessage = "empty_command_type";
        return cmd;
    }

    // Parse command type
    cmd.type = parseCommandType(typePart);
    ...
    // Find action and payload
    int actionSpace = remaining.indexOf(' ');
    String actionPart;
    String payloadPart = "";

    if (actionSpace == -1)
    {
        actionPart = remaining;
    }
    else
    {
        actionPart = remaining.substring(0, actionSpace);
        payloadPart = remaining.substring(actionSpace + 1);
        payloadPart.trim(); // Clean up payload whitespace
    }
    ...
    // Validate action format (should contain only alphanumeric, dots, and underscores)
    for (int i = 0; i < actionPart.length(); i++)
    {
        char c = actionPart.charAt(i);
        if (!isalnum(c) && c != '.' && c != '_')
        {
            cmd.isValid = false;
            cmd.errorMessage = "invalid_action_format";
            return cmd;
        }
    }

    cmd.action = actionPart;
    cmd.payload = payloadPart;
    cmd.isValid = true;
...
{
    // Basic validation - must start with "CMND"
