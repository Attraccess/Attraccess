...
bool NFC::setup() {
  ...
  if (!this->checkHardware(true)) {
    this->logger.error("PN53x board not found");
    return false;
  }
  ...
  this->logger.infof("Factory key is: %s",
                     hexToString(NFC::FACTORY_KEY, 16).c_str());
  return true;
}
...
void NFC::handleCardDetection() {
  ...
  if (this->foundCard) {
    // just try to comminucate with card in any way to check if it is still
    // present
    bool authSuccess =
        this->pn532.ntag424_Authenticate(NFC::FACTORY_KEY, 0, 0x71);
    if (!authSuccess) {
      ...
      this->logger.debugf(
          "Calling card detection callback with presentation time: %d ms",
          presentationTimeMs);
  ...
  bool foundCardUpdate = this->pn532.readPassiveTargetID(
      PN532_MIFARE_ISO14443A, cardDetectedUid, &cardDetectedUidLength, 100);
...
bool NFC::waitForCard(uint32_t timeoutMs) {
  ...
  uint8_t uidDetected = this->pn532.readPassiveTargetID(
      PN532_MIFARE_ISO14443A, uid, &uidLength, timeoutMs);
...
bool NFC::changeKey(uint8_t keyNumber, uint8_t *masterKey, uint8_t *oldKey,
                    uint8_t *newKey) {
  this->logger.info("changeKey started");

  // Step 1: Authenticate with master key
  bool authenticateOldKeySuccess =
      this->pn532.ntag424_Authenticate(masterKey, 0, 0x71);
  ...
  // Step 2: Change key
  bool changeKeySuccess =
      this->pn532.ntag424_ChangeKey(oldKey, newKey, keyNumber);
  if (!changeKeySuccess) {
  ...
  bool authenticateNewKeySuccess =
      this->pn532.ntag424_Authenticate(newKey, keyNumber, 0x71);
...
bool NFC::authenticate(uint8_t keyNumber, uint8_t *key) {
  ...
  bool authenticateSuccess =
      this->pn532.ntag424_Authenticate(key, keyNumber, 0x71);
...
bool NFC::checkHardware(bool logHardwareInfo) {
  uint32_t now = millis();
  ...
  this->logger.info(("Firmware ver. " + String((versiondata >> 16) & 0xFF) +
...
bool NFC::getAvailableKeyNo(uint8_t *uid, uint8_t *uidLength, uint8_t *keyNo) {
  ...
  bool foundCard = this->pn532.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid,
                                                   uidLength, 50);
  if (!foundCard) {
  ...
  // check key 1 to 5, the first one that we can authenticate using factory key
  // is an available key
...
bool NFC::isCardDetectionEnabled() { return this->cardDetectionEnabled; }
