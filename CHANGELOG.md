...
- **ATT-593:** fix balena not parsing `${VAR:-default}` compose env syntax — Grafana admin credentials now use hardcoded defaults; Pushover vars removed from compose env and must be set as device/fleet variables so `monitoring-init` and `grafana` both see them ([ATT-593](https://linear.app/attraccess/issue/ATT-593), [#1410](https://github.com/Attraccess/Attraccess/pull/1410))
